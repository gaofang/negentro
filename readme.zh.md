...
negentro 是一个面向前端重业务代码仓库的仓库知识蒸馏 CLI 工具。它可以将项目中的约定、实现模式以及经过人工确认的决策提炼为可复用的产物，例如 `AGENTS.md` 和 skills。
...
有些仓库知识可以从代码中挖掘出来，有些则不能。negentro 将“尚未解决的歧义”视为一等概念，并通过一次一个问题的方式请求人工确认。

### 2. 结构化输出就是协议
negentro 提供对 JSON 友好的命令输出，让 agent 环境可以通过结构化结果来编排工作流，而不必从终端日志中解析自然语言。
...
### Extraction
negentro 会扫描应用或仓库范围，并生成结构化的候选知识。

### Questions
当仅靠代码无法得出结论时，negentro 会生成人工确认问题。
...
如果你希望开发者直接在 Codex 中驱动 negentro，而不是切换到独立 TUI，可以安装 Codex 集成：
